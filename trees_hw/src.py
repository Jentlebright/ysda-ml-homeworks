...
def gini(y: np.ndarray) -> float:
    ...
    if y.size == 0:
        return 0.0
    vals_not_to_use, counts = np.unique(y, return_counts=True)
    probas = counts / y.size
    gini_index = 1 - np.sum(probas ** 2)
    return gini_index
...
def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> \
    ...
    weighted = (y_left.size / (y_left.size + y_right.size)) * left_impurity + (y_right.size / (y_left.size + y_right.size)) * right_impurity 
    return weighted, left_impurity, right_impurity
...
def create_split(feature_values: np.ndarray, threshold: float) -> tp.Tuple[np.ndarray, np.ndarray]:
    ...
    """
    left_idx, right_idx = [], []
    for idx, elem in enumerate(feature_values):
            if elem <= threshold:
                left_idx.append(idx)
            else:
...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if (
            ...
            self._predicted_class = np.bincount(y).argmax()
            counts = np.bincount(y, minlength=self._meta._n_classes)
            self._class_proba = counts / counts.sum() # vector of probabilities of all classes with index from 0 to n_classes-1 (_n_classes from tree class)
            return self
...
class MyRandomForestClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _bootstrap_sample(self, X: np.ndarray, y: np.ndarray):
        ...
        # bootstrap indices with equal probability from X
...
class ExperimentHandler:
    ...
    def score_cv(self, estimator, metrics, run):
        ...
        :return: None
        """
        cross_val_results = cross_validate(estimator, self.X_train, self.y_train, cv=self._cv_iterable, scoring=metrics, n_jobs=self._n_jobs)
    ...
    def generate_stacking_predictions(self, estimator, run):
        ...
        if hasattr(estimator, "predict_proba"):  # choose the most informative method for stacking predictions
            ...
            method = "predict"
        cross_val_stacking_prediction = cross_val_predict(estimator, self.X_train, self.y_train, cv=self._cv_iterable, method=method, n_jobs=self._n_jobs)
